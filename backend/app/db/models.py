...
class HermesAnalysisJob(Base):
    """A durable cloud-side hand-off to the private macmini Hermes agent."""

    __tablename__ = "hermes_analysis_jobs"
    __table_args__ = (UniqueConstraint("idempotency_key", name="uq_hermes_analysis_job_key"),)
...
class HermesAgentNonce(Base):
    """One-time nonces prevent replay of signed agent requests."""

    __tablename__ = "hermes_agent_nonces"
    __table_args__ = (UniqueConstraint("agent_id", "nonce", name="uq_hermes_agent_nonce"),)
